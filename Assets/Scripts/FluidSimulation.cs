...
public class FluidSimulation : MonoBehaviour
{
    private class Fluid
    {
        ...
        public CustomVector3[] buffer1;
        public CustomVector3[] buffer2;
        ...
        public Fluid(int width, int height, float verticesDistance, float timeSlice, float wavePropagationVelocity, float viscosity)
        {
            ...
            buffer1 = new CustomVector3[count];
            buffer2 = new CustomVector3[count];
            renderBuffer = 0;
        ...
        {
            CustomVector3[] currentBuffer = renderBuffer == 0 ? buffer1 : buffer2;
            CustomVector3[] previousBuffer = renderBuffer == 1 ? buffer1 : buffer2;
        ...
        {
            CustomVector3[] nextBuffer = renderBuffer == 0 ? buffer1 : buffer2;
    ...
    public ComputeShader fluidSimulationCompute;
    ...
    void OnDrawGizmos()
    {
        ...
        CustomVector3[] currentBuffer = fluid.renderBuffer == 0 ? fluid.buffer1 : fluid.buffer2;
    ...
    void FixedUpdate()
    {
        fluidBuffer1 = new ComputeBuffer(widthAndHeight * widthAndHeight, 3 * sizeof(float));
        fluidBuffer2 = new ComputeBuffer(widthAndHeight * widthAndHeight, 3 * sizeof(float));

        fluidBuffer1.SetData(fluid.buffer1);
        fluidBuffer2.SetData(fluid.buffer2);

        fluidSimulationCompute.SetBuffer(0, "CurrentBuffer", fluid.renderBuffer == 0 ? fluidBuffer1 : fluidBuffer2);
        fluidSimulationCompute.SetBuffer(0, "PreviousBuffer", fluid.renderBuffer == 1 ? fluidBuffer1 : fluidBuffer2);
        fluidSimulationCompute.SetTexture(0, "Result", heightMap);
        fluidSimulationCompute.SetFloat("k1", fluid.k1);
        fluidSimulationCompute.SetFloat("k2", fluid.k2);
        fluidSimulationCompute.SetFloat("k3", fluid.k3);
        fluidSimulationCompute.SetInt("width", fluid.width);
        fluidSimulationCompute.SetInt("height", fluid.height);
        fluidSimulationCompute.Dispatch(0, widthAndHeight / 8, widthAndHeight / 8, 1);

        //fluid.Evaluate();

        fluid.SwapBuffers();

        fluidBuffer1.GetData(fluid.buffer1);
        fluidBuffer2.GetData(fluid.buffer2);
        ...
        //        CustomVector3 currentVertex1 = currentBuffer[nearestXDown * widthAndHeight + nearestYDown];
        //        CustomVector3 currentVertex2 = currentBuffer[nearestXDown * widthAndHeight + nearestYUp];
        //        CustomVector3 currentVertex3 = currentBuffer[nearestXUp * widthAndHeight + nearestYUp];
        //        CustomVector3 currentVertex4 = currentBuffer[nearestXUp * widthAndHeight + nearestYDown];
